...
| `energy` | float 0-1 | 0.22 (classical) to 0.97 (metal) | Vibe intensity; the single most important axis |
| `valence` | float 0-1 | 0.22 to 0.84 | Positivity/happiness of the sound |
| `danceability` | float 0-1 | 0.28 to 0.95 | Rhythmic suitability for movement |
| `acousticness` | float 0-1 | 0.03 to 0.97 | Organic/acoustic vs produced/electronic texture |
| `tempo_bpm` | float | 60 to 168 | Pacing in beats per minute |
...
energy_proximity = 1 - |user.target_energy - song.energy|   # 0.0 to 1.0
score += energy_proximity x 1.5                              # rewards closeness, not just high/low

if user.likes_acoustic:
    score += song.acousticness x 1.0    # reward organic texture
else:
    score += (1 - song.acousticness) x 0.5   # mild preference for electronic sound
```
...
- Energy proximity (x1.5) uses `1 - |diff|` so a user wanting `energy=0.35` gets rewarded for low-energy songs, not penalized.
- Acousticness (x1.0 / x0.5) is secondary; it refines texture preference after the big filters have already run.
...
flowchart TD
    ...
    C --> D[Genre match?\n+2.0 if yes]
    C --> E[Mood match?\n+1.5 if yes]
    C --> F[Energy proximity\n1 - abs(target - song.energy) x 1.5]
    C --> G[Acousticness fit\nsong.acousticness x 1.0 or 0.5]
...
**Mood label rigidity:** Mood matching is binary: "chill" is not "relaxed" even though a listener might accept both. A real system would map these to a continuous emotion space (e.g., valence x arousal axes). This system could mis-rank songs that *feel* right but use a slightly different mood label.
